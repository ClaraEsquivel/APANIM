...
const CadastroAnimalPerdido = () => {
    ...
    const handleDataDesaparecimentoChange = (e) => {
        ...
        if (dataValue) {
            ...
            if (dataSelecionada > hoje) {
                alertaAviso('Data Inválida', 'A data de desaparecimento não pode ser uma data futura!');
            ...
            if (diffDays === 0) {
                ...
                setTempoDesaparecido(`⚠️ Desapareceu há ${semanas} semana${semanas > 1 ? 's' : ''}`);
            } else if (diffDays <= 365) {
                const meses = Math.floor(diffDays / 30);
                setTempoDesaparecido(`⏰ Desapareceu há ${meses} ${meses > 1 ? 'meses' : 'mês'}`);
            } else {
                const anos = Math.floor(diffDays / 365);
                setTempoDesaparecido(`⏰ Desapareceu há ${anos} ano${anos > 1 ? 's' : ''}`);
            }
        } else {
            setTempoDesaparecido('');
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.dataDesaparecimento) {
            await alertaErro('Campo Obrigatório', 'Por favor, informe a data de desaparecimento');
            return;
        }
        ...
        const animalData = {
            ...
            tempoDesaparecido: tempoDesaparecido,
            dataCadastro: new Date().toISOString(),
            status: 'ativo'
        };

        try {
            // Dados do animal preparados para envio
            const animalData = {
                ...formData,
                id: Date.now().toString(),
                imagemPrincipal: previewImagem,
            ...
            // TODO: Aqui você deve enviar os dados para uma API
            // exemplo: await fetch('/api/animais-perdidos', { method: 'POST', body: JSON.stringify(animalData) })
            
            console.log('📤 Dados do animal pronto para envio:', animalData);

            // Salvar localmente (fallback) para permitir visualização imediata na listagem
            try {
                const chave = 'animais_perdidos';
                const lista = JSON.parse(localStorage.getItem(chave) || '[]');
                lista.push(animalData);
                localStorage.setItem(chave, JSON.stringify(lista));
                window.dispatchEvent(new CustomEvent('animais-perdidos-updated'));
            } catch (e) {
                console.error('Erro ao salvar em localStorage (perdidos):', e);
            }

            // Mostrar modal de confirmação
            const resultado = await confirmarModal(
                '✅ Animal Cadastrado com Sucesso!',
                'O animal foi cadastrado com sucesso! \n\nDeseja visualizar a página de animais perdidos agora?',
                'success'
            );

            // Se confirmou, navega para a página de animais perdidos
            if (resultado) {
                navigate('/animais-perdidos');
            } else {
    ...
    return (
        <>
            ...
            {/* Conteúdo Central */}
            <div className="central_cadastro_animal">
                {/* Patas decorativas */}
                {[...Array(24)].map((_, i) => (
                ...
                <div className="container_cadastro_animal">
                    ...
                    <form onSubmit={handleSubmit}>
                        {/* Nome do Animal */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="nome-animal" className=" form-label_cadastro_animal">Nome do Animal </label>
                            <input 
                                type="text" 
                                className="form-control_cadastro_animal" 
                                id="nome-animal" 
                                name="nome" 
                        ...
                        {/* Idade */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="idade-animal" className=" form-label_cadastro_animal">Idade Aproximada</label>
                            <input 
                                type="text" 
                                className="form-control_cadastro_animal" 
                                id="idade-animal" 
                        ...
                        {/* Espécie */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="especie-animal" className=" form-label_cadastro_animal">Espécie </label>
                            <select 
                                className="form-control_cadastro_animal" 
                                id="especie-animal" 
                                name="especie"
                        ...
                        {formData.especie && (
                            <div className="form-group_cadastro_animal">
                                <label htmlFor="raca-animal" className=" form-label_cadastro_animal">Raça</label>
                                <select 
                                    className="form-control_cadastro_animal" 
                                    id="raca-animal" 
                                    name="raca" 
                                ...
                                >
                                    ...
                                    {formData.especie === 'cachorro' && racasCachorro.map((raca, index) => (
                        ...
                        {/* Porte */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal">Porte</label>
                            <div className="form-check-group_cadastro_animal">
                                {['pequeno', 'medio', 'grande'].map((porte) => (
                                    <div key={porte} className="form-check_cadastro_animal">
                                        <input 
                                            className="form-check-input_cadastro_animal" 
                                            type="radio" 
                                            name="porte" 
                                            id={`porte-${porte}`}
                                        ...
                                        />
                                        <label className="form-check-label_cadastro_animal" htmlFor={`porte-${porte}`}>
                        ...
                        {/* Sexo */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal">Sexo</label>
                            <div className="form-check-group_cadastro_animal">
                                {['macho', 'femea'].map((sexo) => (
                                    <div key={sexo} className="form-check_cadastro_animal">
                                        <input 
                                            className="form-check-input_cadastro_animal" 
                                            type="radio" 
                                            name="sexo" 
                                        ...
                                        />
                                        <label className="form-check-label_cadastro_animal" htmlFor={`sexo-${sexo}`}>
                        ...
                        {/* Cor */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="cor-animal" className=" form-label_cadastro_animal">Cor Predominante</label>
                            <input 
                                type="text" 
                                className="form-control_cadastro_animal" 
                                id="cor-animal" 
                        ...
                        {/* Vacinado */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal">Vacinado?</label>
                            <div className="form-check-group_cadastro_animal">
                                {['sim', 'nao', 'nao-sei'].map((opcao) => (
                                    <div key={opcao} className="form-check_cadastro_animal">
                                        <input 
                                            className="form-check-input_cadastro_animal" 
                                            type="radio" 
                                            name="vacinado" 
                                        ...
                                        />
                                        <label className="form-check-label_cadastro_animal" htmlFor={`vacinado-${opcao}`}>
                        ...
                        {mostrarVacinas && (
                            <div id="vacinas-grupo">
                                <label className=" form-label_cadastro_animal">Quais vacinas o animal possui?</label>
                                <div className="form-check-group_cadastro_animal">
                                    {['V8 (Óctupla)', 'V10 (Décupla)', 'Antirrábica', 'Tríplice Felina', 'Leucemia Felina'].map((vacina) => (
                                        <div key={vacina} className="form-check_cadastro_animal">
                                            <input 
                                                className="form-check-input_cadastro_animal" 
                                                type="checkbox" 
                                            ...
                                            />
                                            <label 
                                                className="form-check-label_cadastro_animal" 
                        ...
                        {/* Castrado */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal">Castrado?</label>
                            <div className="form-check-group_cadastro_animal">
                                {['sim', 'nao', 'nao-sei'].map((opcao) => (
                                    <div key={opcao} className="form-check_cadastro_animal">
                                        <input 
                                        ...
                                        />
                                        <label className="form-check-label_cadastro_animal" htmlFor={`castrado-${opcao}`}>
                                            {opcao === 'sim' ? 'Sim' : opcao === 'nao' ? 'Não' : 'Não sei'}
                                        </label>
                                    </div>
                                ))}
                        ...
                        {/* Vermifugado */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal">Vermifugado?</label>
                            <div className="form-check-group_cadastro_animal">
                                {['sim', 'nao', 'nao-sei'].map((opcao) => (
                                    <div key={opcao} className="form-check_cadastro_animal">
                                        <input 
                                        ...
                                        />
                                        <label className="form-check-label_cadastro_animal" htmlFor={`vermifugado-${opcao}`}>
                                            {opcao === 'sim' ? 'Sim' : opcao === 'nao' ? 'Não' : 'Não sei'}
                                        </label>
                                    </div>
                                ))}
                        ...
                        {/* Condição Especial */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="condicao-especial" className=" form-label_cadastro_animal">Condição Especial</label>
                            <input 
                                type="text" 
                                className="form-control_cadastro_animal" 
                                ...
                                onChange={handleInputChange}
                            />
                            <small className="form-text_cadastro_animal">Informe se o animal tem alguma necessidade especial</small>
                        </div>

                        {/* Data de Desaparecimento */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="data-desaparecimento" className=" form-label_cadastro_animal">Data de Desaparecimento </label>
                            <input 
                                type="date" 
                                className="form-control_cadastro_animal" 
                            ...
                            />
                            {tempoDesaparecido && (
                                <small className="form-text_cadastro_animal" style={{ 
                                    color: '#dc2626', 
                                    fontWeight: 'bold',
                                    display: 'block',
                                    marginTop: '8px'
                        ...
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="localizacao-animal" className=" form-label_cadastro_animal">Localização (Bairro) </label>
                            <select 
                                className="form-control_cadastro_animal" 
                                ...
                                required
                            >
                                <option value="">Selecione o bairro onde foi visto pela última vez</option>
                            ...
                            <small className="form-text_cadastro_animal">Informe onde o animal foi visto pela última vez</small>
                        </div>

                        {/* Email de Contato */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="email-contato" className=" form-label_cadastro_animal">Email de Contato</label>
                            <input 
                                type="email" 
                                className="form-control_cadastro_animal" 
                                id="email-contato" 
                        ...
                        {/* Telefone de Contato */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="telefone-contato" className=" form-label_cadastro_animal">Telefone de Contato</label>
                            <input 
                                type="tel" 
                                className="form-control_cadastro_animal" 
                                id="telefone-contato" 
                                ...
                                maxLength="15"
                            />
                            <small className="form-text_cadastro_animal">Informe pelo menos um meio de contato (email ou telefone)</small>
                        </div>

                        {/* Upload de Imagem do Animal */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal" htmlFor="imagem-animal">Foto do Animal </label>
                            <small className="form-text_cadastro_animal">Esta será a foto principal exibida nos anúncios (máx. 5MB)</small>
                            <input 
                                type="file" 
                                className="form-control_cadastro_animal" 
                                id="imagem-animal" 
                        ...
                        {/* Mídias Extras */}
                        <div className="form-group_cadastro_animal">
                            <label className=" form-label_cadastro_animal" htmlFor="midias-extras">Fotos e Vídeos Extras (Opcional)</label>
                            <small className="form-text_cadastro_animal">Máximo de 10 arquivos, cada um com até 10MB</small>
                            <input 
                                type="file" 
                                className="form-control_cadastro_animal" 
                                id="midias-extras" 
                        ...
                        {/* Resumo */}
                        <div className="form-group_cadastro_animal">
                            <label htmlFor="resumo-animal" className=" form-label_cadastro_animal">Descrição do Animal (até 100 caracteres) </label>
                            <textarea 
                                className="form-control_cadastro_animal" 
                                id="resumo-animal" 
                                name="resumo" 
                                ...
                                required
                            ></textarea>
                            <small className="form-text_cadastro_animal">
